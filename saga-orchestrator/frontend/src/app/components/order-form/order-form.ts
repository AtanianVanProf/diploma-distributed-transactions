...
export class OrderForm implements OnInit {
  ...
  orderFailed = output<{ order?: OrderResponse; error?: ErrorResponse }>();
  ...
  protected placeOrder(): void {
    ...
    this.orchestratorApi.placeOrder(request).subscribe({
      next: (response) => {
        this.submitting.set(false);
        if (response.status === 'FAILED') {
          this.orderFailed.emit({ order: response });
        } else {
      ...
      error: (err: HttpErrorResponse) => {
        ...
        this.orderFailed.emit({ error: errorResponse });
      }
    });
  }
