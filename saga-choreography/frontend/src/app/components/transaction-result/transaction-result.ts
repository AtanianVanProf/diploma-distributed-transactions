...
export class TransactionResult {
  ...
  result = input<'success' | 'error' | 'timeout' | null>(null);
  ...
  orderId = input<number | undefined>(undefined);
  failureReason = input<string | undefined>(undefined);
  ...
  protected getCustomerBefore(): Customer | undefined {
    const before = this.beforeState();
    if (!before || before.customers.length === 0) return undefined;
    return before.customers[0];
  }

  protected getCustomerAfter(): Customer | undefined {
    const after = this.afterState();
    if (!after || after.customers.length === 0) return undefined;
