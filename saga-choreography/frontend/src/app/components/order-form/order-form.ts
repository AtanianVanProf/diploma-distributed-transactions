...
export class OrderForm implements OnInit {
  ...
  beforeSubmit = output<PlaceOrderRequest>();
  orderSubmitted = output<PlaceOrderResponse>();
  orderError = output<ErrorResponse>();
  ...
  protected placeOrder(): void {
    ...
    this.orderApi.placeOrder(request).subscribe({
      next: (response: PlaceOrderResponse) => {
        ...
        this.orderSubmitted.emit(response);
        this.resetForm();
      },
      error: (err: HttpErrorResponse) => {
        ...
        const errorResponse: ErrorResponse = err.error?.error
          ? err.error
          : { error: 'UNKNOWN', message: err.message || 'An unexpected error occurred', details: {} };
        this.orderError.emit(errorResponse);
      }
    });
  }
